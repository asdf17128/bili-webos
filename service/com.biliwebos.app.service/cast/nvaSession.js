// Hard ceiling on any single advertised body/frame length. NVA control
// messages are tiny JSON blobs; anything claiming more than this is malformed
// or hostile, so we refuse it rather than buffer toward OOM.
var MAX_BODY_LENGTH = 4 * 1024 * 1024;
  ...
  while (this.buffer.length >= 6) {
    ...
    if (paramCount === 0) {
      ...
      if (paramCount === 1) {
        ...
        var replyBodyLen = this.buffer.readUInt32BE(6);
        if (replyBodyLen > MAX_BODY_LENGTH) { this.close(); return; }
        total = 10 + replyBodyLen;
      } else {
        total = 6;
      }
    } else {
      ...
      if (paramCount === 3) {
        ...
        var bodyLen = this.buffer.readUInt32BE(total);
        if (bodyLen > MAX_BODY_LENGTH) { this.close(); return; }
        total += 4 + bodyLen;
      }
    }
